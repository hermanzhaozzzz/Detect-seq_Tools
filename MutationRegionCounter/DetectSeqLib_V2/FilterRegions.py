...
def add_block_info_to_mpmat(
    ...
    try:
        ...
        # open ctrl bmat file
        if (chr_bmat_ctrl_filename[-3:] == ".gz") or (
            chr_bmat_ctrl_filename[-5:] == ".gzip"
        ):
        ...
        if (chr_bmat_treat_filename[-3:] == ".gz") or (
            chr_bmat_treat_filename[-5:] == ".gzip"
        ):
    ...
    for line_index, line in enumerate(chr_mpmat_file):
        ...
        if line_index % run_report_num == 0:
            logging.info(
                "Running mpmat block step on %s \n\tProcessed line number %s"
                % (chr_mpmat_filename, line_index + 1)
            )
        ...
        ctrl_bmat_res_dict = query_region_bmat_info(
            bmat_file=chr_bmat_ctrl_file,
            site_index_list=query_site_idx_list,
            genome_order_dict=ref_order_dict,
        )

        treat_bmat_res_dict = query_region_bmat_info(
            bmat_file=chr_bmat_treat_file,
            site_index_list=query_site_idx_list,
            genome_order_dict=ref_order_dict,
        )
        ...
        for site_idx in ctrl_bmat_res_dict["site_index_list"]:
            if ctrl_bmat_res_dict[site_idx] is None:
                ...
                hard_filter_res = site_hard_filter(
                    site_bmat_obj=ctrl_bmat_res_dict[site_idx],
        ...
        for index, site_idx in enumerate(ctrl_bmat_res_dict["site_index_list"]):
            if not ctrl_site_hard_filter_state[index]:
                if ctrl_site_hard_filter_reason[index] != "NS":
                    binom_test_res = site_binomial_test(
                        bmat_line_obj=ctrl_bmat_res_dict[site_idx],
                        mut_type=mpmat_info.mut_type,
        ...
        for site_idx in treat_bmat_res_dict["site_index_list"]:
            if treat_bmat_res_dict[site_idx] is None:
                ...
                hard_filter_res = site_hard_filter(
                    site_bmat_obj=treat_bmat_res_dict[site_idx],
                    mut_type=mpmat_info.mut_type,
        ...
        for index in range(mpmat_info.site_num):
            ...
            if treat_bmat_res_dict[site_idx] is None:
                ...
                site_mut_count = treat_bmat_res_dict[site_idx].count_dict[
                    mpmat_info.mut_type[-1]
                ]
        ...
        # out block info
        # ---------------------------------------------------------->>>>>>>>>>
        out_block_info_list = ["CHF,CHR,CBT,THF,THR,FBS,FBR"]
